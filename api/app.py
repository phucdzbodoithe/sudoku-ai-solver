...
# --- HÀM MỚI: KIỂM TRA ĐỀ BÀI CÓ HỢP LỆ KHÔNG ---
def check_initial_board(board):
    for r in range(9):
        for c in range(9):
            if board[r][c] != 0:
                num = board[r][c]
                board[r][c] = 0 # Tạm thời xóa số đó đi
                # Kiểm tra xem nếu điền lại số đó vào thì có bị trùng không
                if not is_valid(board, r, c, num):
                    board[r][c] = num # Trả lại số cũ
                    return False # Bị trùng -> Đề sai
                board[r][c] = num # Trả lại số cũ
    return True
...
def api_solve():
    ...
    # BƯỚC 1: KIỂM TRA ĐỀ BÀI TRƯỚC
    if not check_initial_board(board):
        return jsonify({'status': 'invalid'}) # Trả về lỗi nếu đề sai luật
        
    # BƯỚC 2: NẾU ĐỀ ỔN MỚI GIẢI
    if solve(board): 
